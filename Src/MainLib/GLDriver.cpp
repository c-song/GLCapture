...
GLDriver::~GLDriver()
{
  ...
  if(intercept_detail_Log)
  {
	  delete intercept_detail_Log;
	  intercept_detail_Log = NULL;
  }
  if(errorDataLog)
  {
...
bool GLDriver::LogFunctionPre(uint index, const FunctionArgs & args)
{
  ...
  if(interceptLog)
  {
	...
	intercept_detail_Log->LogFunctionPre(funcData,index,args);
  }
...
bool GLDriver::LogFunctionPost(uint index, const FunctionRetValue & returnVal)
{
  ...
  if(interceptLog)
  {
	...
	intercept_detail_Log->LogFunctionPost(funcData,index,returnVal);
  }
  ...
  if(interceptLog)
  {
	...
	intercept_detail_Log->LogFunctionComplete();
  }
...
bool GLDriver::InitFunctionLog()
{
  ...
  else
  {
	...
	intercept_detail_Log = new InterceptDetailedLogText((currLogDir+configData.logName+"_Detail.txt").c_str(),functionTable,configData);
  }
